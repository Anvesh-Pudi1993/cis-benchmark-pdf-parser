...
# Wrap the thieved model in ART's TensorFlowV2Classifier
thieved_classifier = TensorFlowV2Classifier(model=thieved_model, nb_classes=num_classes, input_shape=input_shape, loss_object=tf.keras.losses.CategoricalCrossentropy())

# Assume 'classifier' is the victim model (already created and trained)
...
accuracy_poisoned = np.sum(np.argmax(model_poisoned.predict(X_test), axis=1) == y_test.argmax(axis=1)) / len(y_test)
